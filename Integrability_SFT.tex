...
\end{equation}
The Hamiltonian does not get corrected at first order in perturbation theory because the order $\mu$ string field describing the pp-wave background is constant. However, it seems to get corrected at second order. Indeed, using \StringCode, one can verify that the source to the second order equation of motion $\left[\psi^{(2)}\otimes H^{(0)}\right]$ is non-zero.
