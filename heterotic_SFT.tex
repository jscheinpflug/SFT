...
\int d^2z~\left\langle
	...
	\]
		...
		\section{A $(0,2)$ half-chiral-ring lower bound}\label{sec:02_half_chiral_bound}
		
		In a general $(0,2)$ SCFT one does not have the full $(2,2)$ chiral ring, but supersymmetric marginal perturbations are still organized by the right-moving half-chiral cohomology. Let
		\ie
		\mathcal R_1
		\;:=\;
		\left\{
		\mathcal W:\ (\bar h,\bar q)=\left({1\over2},1\right),~h=1,~\bar Q_+\mathcal W=0
		\right\}/\bar Q_+\text{-exact},
		\fe
		and choose a basis $\{\mathcal W_A\}$ of $\mathcal R_1$. The supersymmetric perturbation is
		\ie\label{eq:02_bound_pert}
		S\;\to\;S+\sum_{A} t^A\int d^2z~\Big(\wt G^-_{-{1\over2}}\mathcal W_A+\wt G^+_{-{1\over2}}\overline{\mathcal W}_A\Big).
		\fe
		Let $\mathcal R_2$ denote the finite-dimensional right-chiral cohomology space into which the supersymmetry-preserving beta-function obstructions project.
		Physically, $\mathcal R_1$ is the space of candidate supersymmetric exactly marginal couplings (the ``variables'' of the deformation problem), while $\mathcal R_2$ is the space of independent supersymmetry-preserving obstruction equations that these couplings can generate under CPT (the corresponding ``equations''). In spacetime terms, $\mathcal R_1$ labels candidate neutral chiral directions and $\mathcal R_2$ labels F-term-like conditions that must vanish for the deformation to remain supersymmetric and conformal.
		
		Conformal perturbation theory defines an obstruction map
		\ie\label{eq:02_kuranishi}
		\kappa:\ \mathbb C^{\dim\mathcal R_1}\to \mathbb C^{\dim\mathcal R_2},
		\qquad
		\kappa^\alpha(t)=g^\alpha{}_{AB}\,t^A t^B+O(t^3),
		\qquad
		\alpha=1,\ldots,\dim\mathcal R_2,
		\fe
		where the coefficients are extracted from integrated NS correlators (3-point and higher-point data). The local supersymmetric conformal manifold is
		\ie\label{eq:02_defspace}
		\textnormal{Def}_{\rm susy}^{(0,2)}
		\;:=\;
		\{t\in\mathbb C^{\dim\mathcal R_1}\,|\,\kappa(t)=0\}
		\;=\;\kappa^{-1}(0).
		\fe
		
		\subsection*{Lemma (Ambient codimension bound)}
		With the definitions above, one always has
		\ie\label{eq:02_basic_bound}
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim\mathcal R_1-\dim\mathcal R_2.
		\fe
		
		\subsection*{Proof}
		By definition, $\textnormal{Def}_{\rm susy}^{(0,2)}$ is the fiber over the origin of the map $\kappa$ in (\ref{eq:02_kuranishi}). For any holomorphic map between finite-dimensional complex spaces,
		\ie
		\dim\kappa^{-1}(0)\;\ge\;\dim(\textnormal{domain})-\dim(\textnormal{image}).
		\fe
		Hence
		\ie
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim\mathcal R_1-\dim(\textnormal{Im}\,\kappa).
		\fe
		Since $\textnormal{Im}\,\kappa\subseteq\mathbb C^{\dim\mathcal R_2}$, we have $\dim(\textnormal{Im}\,\kappa)\le \dim\mathcal R_2$, and therefore
		\ie
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim\mathcal R_1-\dim\mathcal R_2,
		\fe
		which is (\ref{eq:02_basic_bound}). \hfill$\square$
		
		Equivalent tangent-space statement: at any solution $t_0$,
		\[
		T_{t_0}\textnormal{Def}_{\rm susy}^{(0,2)}=\ker(D\kappa_{t_0}),
		\qquad
		\dim\ker(D\kappa_{t_0})\ge \dim\mathcal R_1-\dim\mathcal R_2
		\]
		by rank--nullity.
		
		\subsection*{Proposition (Effective obstruction-subspace reduction)}
		Suppose there exists a finite-dimensional subspace
		\ie
		\mathcal O_{\rm eff}\subseteq \mathcal R_2
		\fe
		such that
		\ie
		\textnormal{Im}\,\kappa\subseteq \mathcal O_{\rm eff}.
		\fe
		Then
		\ie\label{eq:02_intermediate_obstruction_bound}
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim\mathcal R_1-\dim\mathcal O_{\rm eff}.
		\fe
		\textit{Proof.}
		By the codimension estimate used above,
		\[
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim\mathcal R_1-\dim(\textnormal{Im}\,\kappa).
		\]
		Because $\textnormal{Im}\,\kappa\subseteq\mathcal O_{\rm eff}$,
		\[
		\dim(\textnormal{Im}\,\kappa)\le \dim\mathcal O_{\rm eff},
		\]
		which implies (\ref{eq:02_intermediate_obstruction_bound}). \hfill$\square$
		
		\subsection*{Proposition (Generic quotient commuting diagram)}
		Assume a splitting
		\[
		\mathcal R_1=C\oplus B
		\]
		with projection $p:\mathcal R_1\to C$ and inclusion $i_C:C\hookrightarrow \mathcal R_1$. Define
		\[
		\kappa_C:=\kappa\circ i_C:\ C\to \mathcal R_2,
		\qquad
		\Delta\kappa(c,b):=\kappa(c,b)-\kappa(c,0),
		\]
		and let
		\[
		\mathcal O_B^{\rm quo}:=\mathrm{span}\,\mathrm{Im}(\Delta\kappa)\subseteq \mathcal R_2.
		\]
		Let $q_{\rm quo}:\mathcal R_2\to \mathcal R_2/\mathcal O_B^{\rm quo}$ be the quotient map. Then
		\[
		q_{\rm quo}\circ\kappa
		=
		(q_{\rm quo}\circ\kappa_C)\circ p,
		\]
		i.e.\ the diagram
		\[
		\begin{array}{ccc}
		\mathcal R_1 & \xrightarrow{\ \kappa\ } & \mathcal R_2\\
		\downarrow p & & \downarrow q_{\rm quo}\\
		C & \xrightarrow{\ q_{\rm quo}\circ\kappa_C\ } & \mathcal R_2/\mathcal O_B^{\rm quo}
		\end{array}
		\]
		commutes.
		\textit{Proof.}
		For $t=(c,b)\in C\oplus B$,
		\[
		\kappa(t)-\kappa_C(p(t))=\Delta\kappa(c,b)\in\mathcal O_B^{\rm quo}.
		\]
		Applying $q_{\rm quo}$ gives the claim. \hfill$\square$
		
		\textit{Corollary.}
		If $q_{\rm quo}\circ\kappa_C\equiv 0$, then $\mathrm{Im}\,\kappa\subseteq \mathcal O_B^{\rm quo}$.
		\textit{Proof.}
		By the proposition, $q_{\rm quo}\circ\kappa=0$, so every value of $\kappa$ lies in $\ker q_{\rm quo}=\mathcal O_B^{\rm quo}$. \hfill$\square$
		
		\subsection*{Conjecture (Quadratic Ward--TT statement for generic $(0,2)$)}
		Let $C$ be a chosen descendant-type block of $\mathcal R_1$ (for instance the $q=0$ descendant sector (\ref{defmoduli}) in a standard-embedding frame), and let $\kappa_C$ and $q_{\rm quo}$ be as above. Then the projected pure-$C$ obstruction map has no quadratic term:
		\[
		(q_{\rm quo}\circ\kappa_C)(t_C)=O(t_C^3).
		\]
		Equivalently, the quadratic piece obeys
		\[
		\big[q_{\rm quo}\circ\kappa_C\big]_{(2)}=0.
		\]
		This is the generic $(0,2)$ analogue of the Tian--Todorov-type vanishing, formulated only at quadratic order.
		
		\subsection*{Theorem 1 (Conditional quadratic Ward proof)}
		Assume for the chosen descendant block $C$ that:
		\begin{enumerate}
		\item the same supercurrent contour manipulation used in the section 3.2 Ward-identity lemma (\ref{eq:rule:ward-desc-3pt}) applies to the NS--NS--NS correlators that determine the quadratic coefficients in (\ref{eq:cpt:beta2});
		\item after projection by $q_{\rm quo}$, no anomalous contact-term contribution survives from integrated operator collisions at order $O(t_C^2)$.
		\end{enumerate}
		Then
		\[
		\big[q_{\rm quo}\circ\kappa_C\big]_{(2)}=0.
		\]
		\textit{Proof.}
		The quadratic term of $\kappa_C$ is determined by the projected NS--NS--NS OPE coefficients in the pure-$C$ sector. Under assumption (1), the same contour-shrinking argument as in (\ref{eq:rule:ward-desc-3pt}) removes the would-be three-point coefficient, up to contact terms. Assumption (2) removes these residual terms in the quotient channel. Hence the $O(t_C^2)$ coefficient of $q_{\rm quo}\circ\kappa_C$ vanishes. \hfill$\square$
		
		\subsection*{Theorem 2 (Block-collapse theorem)}
		Suppose the marginal half-chiral space admits a decomposition $\mathcal R_1=C\oplus B$, and there exists a finite-dimensional obstruction subspace $\mathcal O_B\subseteq\mathcal R_2$ such that:
		\begin{enumerate}
		\item $\textnormal{Im}\,\kappa\subseteq\mathcal O_B$;
		\item $\mathcal O_B$ is paired non-degenerately with $B$, so $\dim\mathcal O_B=\dim B$.
		\end{enumerate}
		Then
		\ie\label{eq:02_structural_bound}
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}
		\;\ge\;
		\dim(C\oplus B)-\dim\mathcal O_B
		\;=\;
		\dim C
		\fe
		\textit{Proof.}
		Apply Proposition (\ref{eq:02_intermediate_obstruction_bound}) with $\mathcal O_{\rm eff}=\mathcal O_B$, then substitute $\mathcal R_1=C\oplus B$ and $\dim\mathcal O_B=\dim B$. \hfill$\square$
		
		\subsection*{Theorem 3 (Near-$(2,2)$ Anderson-type specialization)}
		Near a standard-embedding $(2,2)$ locus, let $C$ be the complex-structure-type descendant block in Theorem 2 and define
		\ie
		\dim C=:N_{\rm cs}^{(2,2)}.
		\fe
		If, in addition, the pure complex-structure block is unobstructed at the reference $(2,2)$ point and the image-restriction hypothesis $\textnormal{Im}\,\kappa\subseteq\mathcal O_B$ of Theorem 2 holds near that point, then
		\ie
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}\ge N_{\rm cs}^{(2,2)}.
		\fe
		\textit{Proof.}
		Immediate from (\ref{eq:02_structural_bound}) and $\dim C=N_{\rm cs}^{(2,2)}$. \hfill$\square$
		At large volume, $N_{\rm cs}^{(2,2)}=h^{2,1}(X)=h^1(X,T_X)$.
		
		\subsection*{Scope of the structural assumptions}
		The quotient commuting relation above is generic once a split $\mathcal R_1=C\oplus B$ is chosen: it is a formal identity, not a dynamical claim. The nontrivial physics is the TT-type statement for the pure-$C$ block. In a generic abstract $(0,2)$ SCFT we therefore formulate this as the quadratic conjecture and isolate a precise conditional Ward-identity theorem (Theorem 1). The stronger block-collapse bound of Theorem 2 requires additional global input, namely an actual obstruction subspace $\mathcal O_B$ containing $\mathrm{Im}\,\kappa$ and the pairing condition $\dim\mathcal O_B=\dim B$. Without this extra input, one should only use the ambient bound
		\ie
		\dim \textnormal{Def}_{\rm susy,loc}^{(0,2)}\ge \dim\mathcal R_1-\dim\mathcal R_2.
		\fe
		
		\subsection*{Relation to the Atiyah-sequence proof of \cite{Anderson:2011ty}}
		The generic quotient proposition is the abstract CPT counterpart of separating the simultaneous obstruction map into a pure complex-structure part plus a bundle-generated remainder. Theorem 2 is the obstruction-image restriction plus cancellation step in \cite{Anderson:2011ty}, and Theorem 3 is the near-$(2,2)$ specialization. Near a geometric large-volume point, the dictionary is
		\ie
		\mathcal R_1 \leftrightarrow H^1(X,\mathcal Q),\qquad
		\kappa \leftrightarrow \kappa_{(X,V)},\qquad
		\mathcal R_2 \leftrightarrow \textnormal{Im}(\rho)\subseteq H^2(X,\mathcal Q),
		\fe
		with the split
		\ie
		C \leftrightarrow H^1(X,T_X),\qquad
		B \leftrightarrow H^1(X,\textnormal{End}(V)),
		\fe
		and mixed lifting map $\alpha_{\rm CPT}$ corresponding to the Atiyah map $\alpha:H^1(X,T_X)\to H^2(X,\textnormal{End}(V))$. The TT and commuting-diagram identifications are
		\[
		\kappa_X\equiv 0\ \Longleftrightarrow\ q_{\rm quo}\circ\kappa_C\equiv 0,
		\qquad
		\textnormal{commuting Kuranishi square}\ \Longleftrightarrow\ q_{\rm quo}\circ\kappa=(q_{\rm quo}\circ\kappa_C)\circ p.
		\]
		
		More precisely, the assumptions imported from \cite{Anderson:2011ty} are:
		\begin{enumerate}
		\item {\it Geometric setup:} a Calabi--Yau threefold $X$, a holomorphic bundle $V$ with Atiyah extension $0\to \textnormal{End}(V)\to \mathcal Q\to T_X\to 0$, and the associated long exact sequence.
		\item {\it Commuting obstruction diagram:} the Kuranishi maps for $X$, $V$, and $(X,V)$ commute with the maps induced by the extension sequence.
		\item {\it Unobstructed base complex structure:} $\kappa_X$ has vanishing image (Tian--Todorov input), so the simultaneous obstruction image is forced into the bundle side, i.e.\ $\textnormal{Im}\,\kappa_{(X,V)}\subseteq \textnormal{Im}\,\rho$.
		\item {\it Bundle-side pairing:} $h^1(X,\textnormal{End}(V))=h^2(X,\textnormal{End}(V))$, giving the cancellation that collapses the RHS to the single complex-structure term.
		\end{enumerate}
		With this dictionary, the quotient proposition is the formal obstruction-projection statement, Theorem 2 is the resulting single-term block-collapse bound once image restriction and pairing are imposed, and Theorem 3 is the near-$(2,2)$ specialization. At the strict $(2,2)$ point the bound is saturated; its nontrivial content is for nearby genuine $(0,2)$ backgrounds where mixed liftings are turned on.
