...
There is nevertheless one useful universal lesson from the scalar calculation.  Define
\begin{equation}
...
Z^{-\omega}
\end{equation}
is the universal projective-coordinate dressing of a mode with definite global energy $\omega$.  Near the
center of AdS this becomes
\begin{equation}
Z^{-\omega}\sim R^{-\omega}e^{-i\omega X^0/R}.
\end{equation}

So for a more general state, the natural projective-coordinate ansatz is not a pure scalar but rather
\begin{equation}
\Phi_\omega(X)=Z^{-\omega}\times \text{(polynomial/tensor/spinor structure)}.
\end{equation}
For example, an AdS highest-weight state carrying spin in a fixed plane is expected to look schematically
like
\begin{equation}
...
What is universal here is only the $Z^{-\omega}$ time dependence.  The actual relation between $\omega$
and the physical state data, as well as the mixing between different components, is dynamical and cannot be
read off from this factor alone.
